...
from .contracts import Yue2Artifact, Yue2Job
...
class Yue2ArtifactStore:
    """Durable per-job state and generated files."""
    ...
    def job_dir(self, job_id: str) -> Path:
        if not job_id or Path(job_id).name != job_id or any(part in {".", ".."} for part in Path(job_id).parts):
            raise ValueError("invalid YuE2 job id")
        return self.root / job_id

    def create_job(self, job: Yue2Job) -> None:
    ...
    def write_job(self, job: Yue2Job) -> None:
    ...
    def read_job(self, job_id: str) -> dict[str, Any]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"YuE2 job was not found: {job_id}")
        ...
        raise RuntimeError(f"could not read YuE2 job state: {path}")
            ...
            payload.update(
                {
                    ...
                    "progress": 1.0,
                    "failure": {
                        "code": "yue2_worker_interrupted",
                        "message": "YuE2 worker restarted before the job completed",
                        "stage": payload.get("stage") or "runtime",
                        "retryable": False,
                        "attempt": payload.get("attempt", 0),
                    },
                    "failureCode": "yue2_worker_interrupted",
                    "refundRequired": True,
                    "refundReason": "yue2_worker_interrupted_before_completion",
                    "updated_at": utc_now(),
                }
            )
    ...
    def artifact(self, job_id: str, name: str, media_type: str | None = None) -> Yue2Artifact:
