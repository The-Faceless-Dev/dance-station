...
from .artifacts import Yue2ArtifactStore
...
def _public_job_payload(job: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in payload.get("artifacts") or []:
        ...
        clean["downloadUrl"] = f"/v1/yue2/jobs/{quote(job_id, safe='')}/artifacts/{quote(str(item['name']), safe='')}"
        artifacts.append(clean)
    payload["artifacts"] = artifacts
    return payload


def create_yue2_worker_app(config: Yue2Config | None = None, runtime: Yue2Runtime | None = None) -> FastAPI:
    config = config or Yue2Config.from_env()
    store = Yue2ArtifactStore(config.artifact_root)
    interrupted = store.reconcile_interrupted_jobs()
    runtime = runtime or Yue2Runtime(config)
    print({"event": "yue2_worker_starting", "config": config.to_public_dict(), "interruptedJobs": interrupted}, flush=True)
    worker = Yue2Worker(config, runtime=runtime)
    app = FastAPI(title="YuE2-3B Music Worker")
    install_callback_routes(app, worker, config)

    @app.get("/health")
    async def health() -> dict[str, Any]:
        preflight = runtime.preflight()
        return {"ok": bool(preflight.get("ready")), "runtime": "yue2-audio-cpp", "model": config.model_name, "preflight": preflight}
    ...
    async def ready() -> dict[str, Any]:
        ...
        return {"ok": True, "runtime": "yue2-audio-cpp", "model": config.model_name, "preflight": preflight}

    @app.get("/v1/worker/status")
    async def status() -> dict[str, Any]:
        with worker._lock:
            active = list(worker._futures)
        return {"runtime": "yue2-audio-cpp", "model": config.model_name, "preflight": runtime.preflight(), "activeJobs": active}

    @app.post("/v1/yue2/jobs")
    ...
    @app.get("/v1/yue2/jobs/{job_id}/artifacts/{artifact_name}")
    async def get_artifact(job_id: str, artifact_name: str) -> FileResponse:
        try:
            artifact = store.artifact(job_id, artifact_name)
        except (FileNotFoundError, ValueError) as exc:
            raise HTTPException(status_code=404, detail="YuE2 artifact was not found") from exc
    ...
    app.state.yue2_worker = worker
    return app


config = Yue2Config.from_env()
app = create_yue2_worker_app(config)
