"""Publish MuLaCover readiness to the launch server."""
...
def run() -> int:
    ...
    if not callback_url or not token:
        print("[mulacover-heartbeat] disabled: heartbeat configuration is absent", flush=True)
        return 0
    ready_url = _value("MULACOVER_HEARTBEAT_READY_URL", "http://127.0.0.1:8080/ready")
    ...
    while True:
        ...
        payload = {
            ...
            "runtime": "mulacover",
            "model_revision": _value("MULACOVER_MODEL_NAME", "MuLaCover"),
            "state": "ready" if ready else "starting",
            "ready": ready,
            "loaded_models": [_value("MULACOVER_MODEL_NAME", "MuLaCover")],
        ...
        try:
            ...
            with urlopen(request, timeout=10) as response:
                response.read()
            print(f"[mulacover-heartbeat] sent ready={ready}", flush=True)
        except Exception as exc:
            print(f"[mulacover-heartbeat] send_failed error_type={type(exc).__name__} error={exc}", flush=True)
        time.sleep(interval)
