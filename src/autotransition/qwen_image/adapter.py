...
def install_process_routes(app: FastAPI, worker: QwenImageWorker, config: QwenImageConfig) -> FastAPI:
    ...
    app.state.qwen_background_tasks = background_tasks
    ...
    def _background_task_done(task: asyncio.Task[Any]) -> None:
        ...
        try:
            ...
            print(json.dumps({"event": "qwen_image_queue_job_cancelled", "jobId": task.get_name().removeprefix("qwen-image-job-")}), flush=True)
        except Exception as exc:
            print(json.dumps({"event": "qwen_image_queue_job_failed", "jobId": task.get_name().removeprefix("qwen-image-job-"), "errorType": type(exc).__name__, "error": str(exc)}), flush=True)
    ...
    async def process(request: Request) -> dict[str, Any]:
        ...
        task = asyncio.create_task(_run_queue_job(payload, worker, config), name=f"qwen-image-job-{job_id}")
        background_tasks.add(task)
        task.add_done_callback(_background_task_done)
        print(json.dumps({"event": "qwen_image_queue_job_accepted", "jobId": job_id}), flush=True)
        return {"schema_version": 1, "runtime": "qwen-image", "status": "accepted", "id": job_id, "job_id": job_id}
