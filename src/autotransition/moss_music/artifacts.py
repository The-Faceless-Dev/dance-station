...
from .contracts import MossMusicArtifact, MossMusicJob
...
class MossMusicArtifactStore:
    """Durable per-job state and output storage."""
    ...
    def job_dir(self, job_id: str) -> Path:
        if not job_id or Path(job_id).name != job_id or any(part in {".", ".."} for part in Path(job_id).parts):
            raise ValueError("invalid MOSS-Music job id")
        return self.root / job_id

    def create_job(self, job: MossMusicJob) -> None:
    ...
    def write_job(self, job: MossMusicJob) -> None:
            ...
            payload.update(
                {
                    ...
                    "progress": 1.0,
                    "failure": {
                        "code": "moss_music_worker_interrupted",
                        "message": "MOSS-Music worker restarted before the job completed",
                        "stage": payload.get("stage") or "load_backend",
                        "retryable": False,
                        "attempt": payload.get("attempt", 0),
                    },
                    "failureCode": "moss_music_worker_interrupted",
                    "refundRequired": True,
                    "refundReason": "moss_music_worker_interrupted_before_completion",
                    "updated_at": utc_now(),
                }
            )
    ...
    def artifact(self, job_id: str, name: str, media_type: str | None = None) -> MossMusicArtifact:
        path = self.job_dir(job_id) / "final" / Path(name).name
        if not path.is_file():
            raise FileNotFoundError(path)
        return MossMusicArtifact(
            name=path.name,
            path=str(path),
