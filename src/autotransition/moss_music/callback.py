...
def install_callback_routes(app: FastAPI, worker: MossMusicWorker, config: MossMusicConfig) -> FastAPI:
    ...
    app.state.moss_background_tasks = background_tasks
    ...
    async def process(request: Request) -> dict[str, Any]:
        ...
        task = asyncio.create_task(run_queue_job(payload, worker, config), name=f"moss-job-{job_id}")
        background_tasks.add(task)
        task.add_done_callback(_background_task_done)
        print(json.dumps({"event": "moss_queue_job_accepted", "jobId": job_id}), flush=True)
        return {"schema_version": 1, "runtime": "moss-music", "status": "accepted", "id": job_id, "job_id": job_id}
