...
from .contracts import QwenImageEditLoRA


def prepare_loras(
    request_loras: tuple[QwenImageEditLoRA, ...],
    destination: Path,
    config: QwenImageEditConfig,
    emit,
) -> tuple[QwenImageEditLoRA, ...]:
    destination.mkdir(parents=True, exist_ok=True)
    prepared: list[QwenImageEditLoRA] = []
    for index, lora in enumerate(request_loras):
        ...
        if lora.path is not None:
            ...
            source = "https"
        prepared_item = QwenImageEditLoRA(lora.source_url, lora.file_name, lora.scale, target)
