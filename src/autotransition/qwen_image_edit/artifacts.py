...
class QwenImageEditArtifactStore:
    ...
    def job_dir(self, job_id: str) -> Path:
        if not job_id or Path(job_id).name != job_id or any(part in {".", ".."} for part in Path(job_id).parts):
            raise ValueError("invalid Qwen Image Edit job id")
    ...
    def write_job(self, job: dict[str, Any]) -> None:
        ...
        self._atomic_json(self.job_dir(str(job["id"])) / "job.json", job)

    def read_job(self, job_id: str) -> dict[str, Any]:
        path = self.job_dir(job_id) / "job.json"
        if not path.is_file():
            raise FileNotFoundError(f"Qwen Image Edit job was not found: {job_id}")
    ...
    def reconcile_interrupted_jobs(self) -> list[str]:
        ...
        for path in self.root.glob("*/job.json"):
            ...
            payload.update({
                ...
                "progress": 1.0,
                "failure": {
                    "code": "qwen_image_edit_worker_interrupted",
                    "message": "Qwen Image Edit worker restarted before the job completed",
                    "stage": payload.get("stage") or "model_preflight",
                    "retryable": False,
                    "attempt": payload.get("attempt", 0),
                },
                "failureCode": "qwen_image_edit_worker_interrupted",
                "refundRequired": True,
                "refundReason": "qwen_image_edit_worker_interrupted_before_completion",
                "updated_at": utc_now(),
            })
    ...
    def artifact(self, job_id: str, name: str, media_type: str | None = None) -> dict[str, Any]:
